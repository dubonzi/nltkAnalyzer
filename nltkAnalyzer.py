...
class TxtRequirementProcessor(object):
    ...
    def list_wordtypes_from_lemmatized_list(self, lemmatized_list_by_verb_noun_adj_adv, number_of_cat):
    ...
    def create_temp_files_named_by_wordtypes(self, wordtype_categories, categories_directory):
        ...
        for i in range(len(wordtype_categories)):
            category_tmp_file_list.append(NamedTemporaryFile(suffix='wordtype_'+wordtype_categories[i], prefix='.txt', dir=categories_directory)) 
    ...
    def assign_temp_files_txt_content(self, wordtype_categories, lemmatized_list_by_verb_noun_adj_adv):
        category_tmp_file_content = []
        boolean_for_content_test = ''
        for i in range(len(wordtype_categories)):
            category_tmp_file_content.append([w for w in lemmatized_list_by_verb_noun_adj_adv if w==wordtype_categories[i]])
    ...
    def assign_temp_content_to_temp_files(self, category_tmp_file_content, category_tmp_file_list):
        boolean_list = []
    ...
    def plot_txt_results(self, lemmatized_list_by_verb_noun_adj_adv, number_of_cat):
    ...
    def remove_categories_directory(self, categories_directory, tmp_root, tmp_folderid):
...
class HtmlRequirementProcessor(object):
    ...
    def split_html_entities(self, raw_text_and_space):
        raw_text_free_from_html_entity = unicode(BeautifulStoneSoup(raw_text_and_space, convertEntities='html'))
    ...
    def assign_temp_files_html_content(self, wordtype_categories, lemmatized_list_by_verb_noun_adj_adv):
        category_tmp_file_content = []
        boolean_for_content_test = ''
        for i in range(len(wordtype_categories)):
            category_tmp_file_content.append([w for w in lemmatized_list_by_verb_noun_adj_adv if w==wordtype_categories[i]])
